...
if (!apiKey) {
    ...
    console.error('Zorg ervoor dat de GEMINI_API_KEY in je environment staat: export GEMINI_API_KEY="jouw-key"');
    console.error('Of voeg hem toe aan een .env file.');
    process.exit(1);
}
...
if (!query) {
    ...
    console.log('  node run_deep_research.js "Je researchsonderwerp hier"');
...
async function main() {
    try {
        ...
        if (!interactionId) {
            throw new Error('No Interaction ID ontvangen van de API.');
        }

        console.log(`\x1b[32m✅ Deep Research Sessie Gestart!\x1b[0m ID: \x1b[90m${interactionId}\x1b[0m`);
        console.log(`\x1b[33m⏳ Performing in-depth search and analysis (this may take a few minutes)... \x1b[0m`);
        ...
        while (!finished) {
            ...
            try {
                ...
                if (status === 'completed') {
                    ...
                    process.stdout.write('\r\x1b[32m✅ Analyse completed!\x1b[0m                        \n');
                    result = statusRes;
                    finished = true;
                } else if (status === 'failed') {
                    clearInterval(interval);
                    process.stdout.write('\r\x1b[31m❌ Analyse failed!\x1b[0m                       \n');
        ...
        if (!finalReport) {
            console.log('\x1b[33m⚠️  No tekstuele output gegenereerd door het model.\x1b[0m');
            process.exit(0);
        }
