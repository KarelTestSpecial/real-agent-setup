...
if (!apiKey) {
    console.error('\x1b[31m❌ Fout: Geen GEMINI_API_KEY gevonden!\x1b[0m');
    console.error('Zorg ervoor dat de GEMINI_API_KEY in je environment staat: export GEMINI_API_KEY="jouw-sleutel"');
    console.error('Of voeg hem toe aan een .env bestand.');
    process.exit(1);
}
...
if (!query) {
    console.log('\x1b[34mℹ️  Gebruik:\x1b[0m');
    console.log('  node run_deep_research.js "Je onderzoeksonderwerp hier"');
    console.log('\n\x1b[33mVoorbeeld:\x1b[0m');
    console.log('  node run_deep_research.js "Geef een gedetailleerde analyse van de EU AI Act compliance regels voor chatbots vanaf augustus 2026"');
    process.exit(0);
}

console.log(`\n\x1b[36m🧠 Start Deep Research op:\x1b[0m "\x1b[1m${query}\x1b[22m"`);
console.log(`\x1b[90mSleutel gedetecteerd (begint met: ${apiKey.substring(0, 8)}...)\x1b[0m\n`);
...
async function main() {
    try {
        ...
        console.log(`\x1b[32m✅ Deep Research Sessie Gestart!\x1b[0m ID: \x1b[90m${interactionId}\x1b[0m`);
        console.log(`\x1b[33m⏳ Bezig met diepgaand zoeken en analyseren (dit kan enkele minuten duren)... \x1b[0m`);
        ...
        while (!finished) {
            ...
            try {
                ...
                if (status === 'completed') {
                    ...
                    process.stdout.write('\r\x1b[32m✅ Analyse voltooid!\x1b[0m                        \n');
                    result = statusRes;
                    finished = true;
                } else if (status === 'failed') {
                    clearInterval(interval);
                    process.stdout.write('\r\x1b[31m❌ Analyse mislukt!\x1b[0m                       \n');
                    throw new Error(statusRes.error?.message || 'Gemini Deep Research gaf een onbekende fout.');
                } else if (status === 'cancelled') {
                    clearInterval(interval);
                    process.stdout.write('\r\x1b[31m❌ Analyse geannuleerd!\x1b[0m                   \n');
                    throw new Error('Sessie werd geannuleerd.');
                }
            } catch (pollError) {
                // If temporary network error, log and keep trying
                process.stdout.write(`\r\x1b[33m⚠️  Verbindingsprobleem, opnieuw proberen...\x1b[0m`);
            }
        }
        ...
        if (!finalReport) {
            console.log('\x1b[33m⚠️  Geen tekstuele output gegenereerd door het model.\x1b[0m');
            process.exit(0);
        }
        ...
        console.log('\x1b[36m📝 Preview van het rapport:\x1b[0m');
        console.log(finalReport.substring(0, 800) + (finalReport.length > 800 ? '\n\n[... Rapport loopt verder in het opgeslagen bestand ...]' : ''));
